...
$.fn.roundSlider.prototype.drawRange = function (i, start, end, bgcolor) {
  ...
  $svg.attr({ "height": d, "width": d, "viewBox": "0 0 259 259" });

  var $filter = $(document.createElementNS(svgNS, "filter")).attr({
    "id": "dropshadow"
  }).appendTo($svg);

  var $feGaussianBlur = $(document.createElementNS(svgNS, "feGaussianBlur")).attr({
    "in": "SourceAlpha",
    "stdDeviation": "2",
    "result": "blur"
  }).appendTo($filter);

  var $feOffset = $(document.createElementNS(svgNS, "feOffset")).attr({
    "in": "blur",
    "dx": "2",
    "dy": "2",
    "result": "offsetblur"
  ...
  var $feComponentTransfer = $(document.createElementNS(svgNS, "feComponentTransfer")).appendTo($filter);

   var $feFuncA = $(document.createElementNS(svgNS, "feFuncA")).attr({
     "type": "linear",
     "slope": "0.15"
  }).appendTo($feComponentTransfer);

  var $feMerge = $(document.createElementNS(svgNS, "feMerge")).appendTo($filter);

  var $feMergeNode1 = $(document.createElementNS(svgNS, "feMergeNode")).appendTo($feMerge);

  var $feMergeNode2 = $(document.createElementNS(svgNS, "feMergeNode")).attr({
    "in": "SourceGraphic"
  }).appendTo($feMerge);

  var $circle = $(document.createElementNS(svgNS, 'circle')).attr({
    "fill": "transparent", "class": "rs-transition",
    "cx": r, "cy": r, "r": r1,
    "stroke-width": o.width, "stroke": bgcolor,
    "stroke-dasharray": this._circum,
    "filter": "url(#dropshadow)"
  }).appendTo($svg);

  var startPoint = this._valueToAngle(start),
      endPoint = this._valueToAngle(end) - startPoint;

  this.$svg_box = $(document.createElement("div")).addClass("rs-svg rs-transition").append($svg).css({
    "height": d, "width": d, "transform-origin": "50% 50%",
    "transform": "rotate(" + (startPoint + 180) + "deg)"
  }).appendTo(this.innerContainer);

  var pct = (1 - (endPoint / 360)) * this._circum;
  pct = 639;

  $circle.css({ strokeDashoffset: pct });
  this["$circle"+i] = $circle[0];
}
...
$.fn.roundSlider.prototype._setProperties = function () {
  ...
  for(var i = 0; i < o.ranges.length; i++) {
  	var obj = o.ranges[i];
    this.drawRange(i, obj.start, obj.end, "transparent");
  }
}
